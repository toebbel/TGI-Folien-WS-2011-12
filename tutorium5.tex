...
\begin{frame}
	...
	$$T_\M(n) = max \menge{\vphantom{1cm} \text{\# Berechnungsschritte bei Eingabe $x$}}{x \in \Sigma^*, \abs{x} = n}$$
\end{frame}
...
Wir definieren eine nichtdeterministische Turingmaschine als Erweiterung einer deterministischen.
 Wir erweiteren die TM um ein \emph{Orakelmodul}, das vor der deterministischen Berechnung Zeichen auf das Band links von der Eingabe schreibt. Danach verfährt die Maschine wie eine normale TM.
...
\frametitle{Nichtdeterministische Turingmaschinen: Zeitkomplexität}
	...
	$$T_\M(n) = max \menge{\vphantom{1cm} \text{\textbf{min.} \# Berechnungsschritte bei Eingabe $x$}}{x \in \Sigma^*, \abs{x} = n}$$
\end{frame}
