...
f"ur $V \in \mathcal{V}$ \\ $\Rightarrow$ "`Vorwegnahme"' dieser Produktionen: Für jede Produktion mit einem der obigen $V$
...
\begin{column}{0.3\textwidth}
\begin{align*}
\textcolor{blue}{\mathcal{S}} &\rightarrow \mathcal{S'} \mid \varepsilon \\
...
\begin{column}{0.3\textwidth}
\begin{align*}
\mathcal{S} &\rightarrow \mathcal{S'} \mid \varepsilon \\
...
\setcounter{enumi}{3}
\item
Sind alle Zyklen beseitigt, werden im letzten Schritt alle Regeln, die rechts eine einzelne Variable haben, durch "`Vorziehen"' der Regeln eliminiert.
\end{enumerate}
